...
contract MasterChef01 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PANTHER
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. PANTHER to distribute per block.
        uint256 lastRewardBlock;    // Last block number that PANTHER distribution occurs.
        uint256 accPantherPerShare; // Accumulated PANTHER\ per share, times 1e12. See below.
    ...
    uint256 public constant MAX_PANTHER_PER_BLOCK = 2000 * 10 ** 18;
    // Bonus multiplier for early panther makers.
    ...
    // Total PANTHER in PANTHER Pools (can be multiple pools)
    uint256 public totalPantherInPools = 0;
    // Panther referral contract address.
    IAnchorReferral public pantherReferral;
    ...
    //actual panther lef in MasterChef can be used in rewards, must excluding all in panther pools
    //this function is for safety check only not used anywhere
    function remainRewards() external view returns (uint256) {
        return panther.balanceOf(address(this)).sub(totalPantherInPools);
    }
    ...
    // View function to see pending Panther on frontend.
    ...
    // View function to see if user can harvest Panther.
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.accPantherPerShare = pool.accPantherPerShare.add(pantherReward.mul(1e12).div(pool.totalLp));
            pool.lastRewardBlock = block.number;

            totalReward.add(pantherReward.div(10));
        }

        safePantherTransfer(devAddress, totalReward);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        safePantherTransfer(devAddress, pantherReward.div(10));
    }
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (address(pool.lpToken) == address(panther)) {
                    totalPantherInPools = totalPantherInPools.add(_amount).sub(depositFee);
                }
            } else {
                ...
                if (address(pool.lpToken) == address(panther)) {
                    totalPantherInPools = totalPantherInPools.add(_amount);
                }
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(panther)) {
                totalPantherInPools = totalPantherInPools.sub(_amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(panther)) {
            totalPantherInPools = totalPantherInPools.sub(amount);
        }
    ...
    // Safe Panther transfer function, just in case if rounding error causes pool do not have enough Panther.
    function safePantherTransfer(address _to, uint256 _amount) internal {
        if (panther.balanceOf(address(this)) > totalPantherInPools) {
            //pantherBal = total panther in MasterChef - total panther in panther pools, this will make sure that MasterChef never transfer rewards from deposited panther pools
            uint256 pantherBal = panther.balanceOf(address(this)).sub(totalPantherInPools);
    ...
    function setPantherReferral(IAnchorReferral _pantherReferral) public onlyOwner {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(pantherReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                safePantherTransfer(referrer, commissionAmount);
    ...
    function updateEmissionRate(uint256 _pantherPerBlock) public onlyOperator {
        require(_pantherPerBlock <= MAX_PANTHER_PER_BLOCK, "PANTHER per block too high");
        massUpdatePools();
